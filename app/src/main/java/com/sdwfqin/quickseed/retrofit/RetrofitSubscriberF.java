 ...
 * @date 2018/4/4
 */
public abstract class RetrofitSubscriberF<T> implements Subscriber<T> {

    private final WeakReference<BaseFragment> mContent;

    public RetrofitSubscriberF(BaseFragment context) {
        super();
    ...
    public void onNext(T response) {
        if (response instanceof BaseResponse) {
            if (((BaseResponse) response).isOk(mContent.get().getActivity())) {
                onSuccess(response);
            } else {
